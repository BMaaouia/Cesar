...
QString classementstring=QString::number(classement);
    query.prepare("INSERT INTO GESTIONJOUEUR (ID, CLASSEMENT, NOM,PRENOM,DOMAINEDESPORT,NATIONALITE) "
...
bool Joueur::supprimer(int id )
    ...
    query.prepare("Delete from GESTIONJOUEUR where id = :id ");
    ...
    model->setQuery("select * from GESTIONJOUEUR");
    ...
    query.prepare("UPDATE GESTIONJOUEUR set ID= :ID , CLASSEMENT= :CLASSEMENT , NOM= :NOM , PRENOM= :PRENOM, DOMAINEDESPORT= :DOMAINEDESPORT, NATIONALITE= :NATIONALITE where ID= :ID" );
    query.bindValue(":ID", idString);
    query.bindValue(":CLASSEMENT", classementString);
    query.bindValue(": NOM", nom);
    query.bindValue(":PRENOM", prenom);
    ...
    QSqlQueryModel * Joueur::tri()
    {
        ...
        q->prepare("SELECT * FROM GESTIONJOUEUR order by classement asc");
        q->exec();
        model->setQuery(*q);
        return  model;
    }
    ...
    QSqlQueryModel * Joueur::rechercher(QString rech)
    {
            QSqlQueryModel * model= new QSqlQueryModel();
                ...
                model->setQuery("select * from GESTIONJOUEUR where id LIKE '"+rech+"%'");
            return model;
    }
